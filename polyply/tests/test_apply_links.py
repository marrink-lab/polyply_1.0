...
class TestApplyLinks:
    ...
    def test_gen_link_fragments(edges, orders, node, ref_edges):
        graph = nx.Graph()
        graph.add_edges_from(edges)
        result_graph, paths = polyply.src.apply_links.gen_link_fragments(graph, orders, node)
    ...
    def test_get_links(lines, ref_ids):
        ...
        meta_mol = MetaMolecule.from_monomer_seq_linear(ff, [Monomer(resname="PEO", n_blocks=5)], "test")
        new_meta_mol = polyply.src.map_to_molecule.MapToMolecule().run_molecule(meta_mol)

        resids = []
        for edge in meta_mol.edges:
            links, ids = polyply.src.apply_links._get_links(meta_mol, edge)
            resids += ids
        print(resids)
    ...
    def test_add_interaction_and_edge(links, interactions, edges, idx, inttype):
        ...
        polyply.src.apply_links.apply_link_between_residues(new_mol, FF.links[0], idx)
    ...
    def test_add_explicit_link(links, interactions, edges, inttype):
        ...
        polyply.src.apply_links.apply_explicit_link(new_mol, FF.links[0])
        assert new_mol.interactions[inttype] == interactions
        assert len(new_mol.edges) == edges
    ...
    def test_explicit_link_failure(links, Error):
        lines = """
        [ moleculetype ]
        GLY  1
        [ atoms ]
