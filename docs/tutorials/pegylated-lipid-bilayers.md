...
```bash
polyply gen_params -f martini_v3.0.0_phospholipids_v1.itp -lib martini3 -o pegylated_lipid.itp -name PEL -seq <lipid-name>:1 EO:<number of monomers>
```
...
monomers of PEO (i.e. `EO:<number of monomers>`). The lipid name is the molecule name
...
```bash
polyply gen_params -f martini_v3.0.0_phospholipids_v1.itp -lib martini3 -o PEL.itp -name PEL -seq POPE:1 EO:50 OHter:1
```
...
```bash
polyply gen_coords -p system.top -o struct.gro -res EO OHter -c bilayer.gro -name test -box 8.0 8.0 20.0 -split POPE:HEAD-NH3:TAIL-PO4,GL1,GL2,C1A,D2A,C3A,C4A,C1B,C2B,C3B,C4B
```

To generate the structure we always need to specify the initial coordinates of the lipid
bilayer (i.e. `-c bilayer.gro`), the residues that need to be built (`-res EO OHter`), and
...
```bash
polyply gen_coords -p system.top -o struct.gro -res EO OHter -c bilayer_brush.gro -name test -box 8.0 8.0 33.0 -split POPE:HEAD-NH3:TAIL-PO4,GL1,GL2,C1A,D2A,C3A,C4A,C1B,C2B,C3B,C4B -b build_file.bld
```
