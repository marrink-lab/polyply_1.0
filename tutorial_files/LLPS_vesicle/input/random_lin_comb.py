	 ...
	 # per back-bone otherwise it's a tree
	     ...
	     # we add them until we draw a stop
             for sc_idxs in range(1, max_mon_side):
                 choice = random.choices(["stop", "add"], weights=[1-p_side_chain[sc_idxs], p_side_chain[sc_idxs]])[0]
                 if choice == "stop":
                     break
                 else:
...
def probability_by_degree_of_polymerization(p, N):
    ...
    N based on the conversion p for linear condensation reaction.
    """
...
random_sample = np.random.choice(weights, p=weight_distribution/np.sum(weight_distribution), size=int(nmols))
